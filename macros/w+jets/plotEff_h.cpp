...
void plotEff_h(){
  ...
  if( sintheta ){
    ...
    hist_leg->AddEntry(h, Form("Z#rightarrow#tau_{h}#tau_{h}"));
    ...
    Tl.SetTextAlign(12);
    Tl.SetTextSize(0.05);
    Tl.DrawLatexNDC(0.15,0.85,"hh final state");

    //save plot
    if (saveplots){
      c101->SaveAs("plots/20161011/w+jets/semilep/Sin_theta_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if( eta ){
    ...
    hist_leg->SetTextFont(62);
    hist_leg->SetBorderSize(0);
    TH1F *h		= (TH1F*)f->Get("361510/plotEventAlgs/taus/Eta_semilep_had");
    ...
    h10->GetYaxis()->SetLabelFont(62); // Absolute font size in pixel (precision 3)
    h10->GetYaxis()->SetLabelSize(0.1);
    h10->GetYaxis()->SetRangeUser(0.,1.2);
    ...
    //save plot
    if (saveplots){
      c99->SaveAs("plots/20161011/w+jets/semilep/Eta_h_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if( pt ){
    ...
    hist_leg->SetTextFont(62);
    hist_leg->SetBorderSize(0);
    TH1F *h		= (TH1F*)f->Get("361510/plotEventAlgs/taus/Pt_semilep_had");
    ...
    //save plot
    if (saveplots){
      c98->SaveAs("plots/20161011/w+jets/semilep/Pt_h_cumulative_Ztautau_vs_wplusmunu_h.pdf");
    }    
  }
  ...
  if( phistar ){
    ...
    //save plot
    if (saveplots){
      c96->SaveAs("plots/20161011/w+jets/semilep/PhiStar_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if( phiacop ){
    ...
    //save plot
    if (saveplots){
      c97->SaveAs("plots/20161011/w+jets/semilep/PhiAcop_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if(  eff ){
    ...
    hist_leg1->AddEntry(dt3, Form("p^{h}_{T}"),"l");
    hist_leg1->AddEntry(dt4, Form("#eta^{h}"),"l");
    ...
    Tl.SetTextAlign(12);
    Tl.SetTextSize(0.03);
    Tl.DrawLatexNDC(0.2,0.82,"Z#rightarrow#tau_{h}#tau_{h} (sig) vs W(#rightarrow#mu#nu)+jets(#rightarrow#tau_{h}) (bkg)");

    //save plot
    if (saveplots){
      c20->SaveAs("plots/20161011/w+jets/semilep/Eff_h_Ztautau_vs_wplusmunu.pdf");
    }    
