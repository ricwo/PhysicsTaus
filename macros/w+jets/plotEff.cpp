...
void plotEff(){
  ...
  if( aT ){
    ...
    hist_leg->AddEntry(h2, Form("W(#rightarrow#mu#nu)+jets(#rightarrow#tau_{h})"));
    ...
    h10->GetYaxis()->SetRangeUser(.01,3.5);
    ...
    //save plot
    if (saveplots){
      c102->SaveAs("plots/20161016/w+jets/semilep/At_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if( sintheta ){
    ...
    TH1F *h2       	= (TH1F*)f1->Get("361101/plotEventAlgs/taus/Sin_theta_semilep_bkg");
    int rebinval = 1;
    //    TGaxis::SetMaxDigits(3); 
    ...
    h10->GetYaxis()->SetLabelFont(62); // Absolute font size in pixel (precision 3)
    h10->GetYaxis()->SetLabelSize(0.1);
    h10->GetYaxis()->SetRangeUser(-.1,1.4);
    ...
    //save plot
    if (saveplots){
      c101->SaveAs("plots/20161016/w+jets/semilep/Sin_theta_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if( eta ){
    ...
    hist_leg->SetTextFont(62);
    hist_leg->SetBorderSize(0);
    TH1F *h		= (TH1F*)f->Get("361510/plotEventAlgs/taus/Eta_semilep_lep");
    ...
    h10->GetYaxis()->SetLabelFont(62); // Absolute font size in pixel (precision 3)
    h10->GetYaxis()->SetLabelSize(0.1);
    h10->GetYaxis()->SetRangeUser(0.5,1.2);
    ...
    //save plot
    if (saveplots){
      c99->SaveAs("plots/20161016/w+jets/semilep/Eta_l_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if( pt ){
    ...
    //save plot
    if (saveplots){
      c98->SaveAs("plots/20161016/w+jets/semilep/Pt_l_cumulative_Ztautau_vs_wplusmunu_h.pdf");
    }    
  }
  ...
  if( phistar ){
    ...
    //save plot
    if (saveplots){
      c96->SaveAs("plots/20161016/w+jets/semilep/PhiStar_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if( phiacop ){
    ...
    h10->GetYaxis()->SetLabelFont(62); // Absolute font size in pixel (precision 3)
    h10->GetYaxis()->SetLabelSize(0.1);
    h10->GetYaxis()->SetRangeUser(0,15.1);
    ...
    //save plot
    if (saveplots){
      c97->SaveAs("plots/20161016/w+jets/semilep/PhiAcop_cumulative_Ztautau_vs_wplusmunu.pdf");
    }    
  }
  ...
  if(  eff ){
    ...
    Tl.SetTextAlign(12);
    Tl.SetTextSize(0.03);
    Tl.DrawLatexNDC(0.2,0.82,"Z#rightarrow#tau_{h}#tau_{l} (sig) vs W(#rightarrow#mu#nu)+jets(#rightarrow#tau_{h}) (bkg)");

    //save plot
    if (saveplots){
      c20->SaveAs("plots/20161016/w+jets/semilep/Eff_l_Ztautau_vs_wplusmunu.pdf");
    }    
