...
void plotEta(){
  ...
  if (eta){
    ...
    h->Sumw2();
    TH1F *h2       	= (TH1F*)f1->Get("410000/plotEventAlgs/taus/Eta_semilep_had");
    h2->Sumw2();

    int rebinval = 2;
    //    TGaxis::SetMaxDigits(3); 
    h->SetTitle("Reconstructed #eta^{h} distributions");
    h->GetXaxis()->SetTitle("#eta^{h}");
    h->GetYaxis()->SetTitle("Normalised events / 0.2");
    ...
    //save plot
    if (saveplots){
      c9->SaveAs("plots/20161005/Eta_h_Ztautau_vs_ttbar.pdf");
    }    
  }
