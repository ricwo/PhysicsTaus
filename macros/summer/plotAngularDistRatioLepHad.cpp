...
void plotAngularDistRatioLepHad(){
  ...
  if (angular_fraction) {
    ...
    hist_leg = new TLegend(0.69,0.58,0.80,0.88);
    ...
    TH1F *h0		= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/DPhi_tau_lep_had");
    TH1F *h1       	= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/Phi_acop_lep_had_tau_tau");
    TH1F *h2       	= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/Phi_acop_lep_had");
    TH1F *h4       	= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/DPhi_truth_reco_lep_lep_had");
    TH1F *h5       	= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/DPhi_truth_reco_had_lep_had");
    h0->Rebin(20);
    h1->Rebin(20);
    h2->Rebin(20);
    h4->Rebin(20);
    ...
    hist_leg->AddEntry(h0, Form("x_{1} = #Delta#phi_{#tau^{truth}l,had_{obs}}"));
    hist_leg->AddEntry(h1, Form("x_{2} = #phi_{acop}^{lh truth}"));
    hist_leg->AddEntry(h2, Form("x_{3} = #phi_{acop}^{lh obs.}"));
    hist_leg->AddEntry(h4, Form("x_{4} = #Delta#phi_{l obs}^{l truth}"));
    hist_leg->AddEntry(h5, Form("x_{5} = #Delta#phi_{h obs}^{h truth}"));
    ...
    //save plot
    if (saveplots){
      c->SaveAs("plots/20160629/semileptonic/Angular_dists_lep_had_fraction.pdf");
    }    
  }
  ...
  if (angular_fraction) {
    ...
    gStyle->SetTitleX(0.5);
    gStyle->SetTitleAlign(23);
    hist_leg = new TLegend(0.67,0.58,0.78,0.88);
    hist_leg->SetFillColor(0);
    hist_leg->SetLineColor(1);
    hist_leg->SetShadowColor(0);
    hist_leg->SetTextSize(0.05);
    hist_leg->SetTextFont(62);
    hist_leg->SetBorderSize(0);
    TH1F *h0		= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/DPhi_sin_theta_truth_reco_lep_had");
    TH1F *h1       	= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/Phi_truth_lep_had");
    TH1F *h2       	= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/Phi_reco_lep_had");
    ...
    hist_leg->AddEntry(h0, Form("x_{1} = #Delta#phi_{#tau^{truth}l,had}sin#theta^{*}_{#eta}"));
    hist_leg->AddEntry(h1, Form("x_{2} = #phi_{#eta}^{* lh truth}"));
    hist_leg->AddEntry(h2, Form("x_{3} = #phi_{#eta}^{* lh obs.}"));
    ...
    //save plot
    if (saveplots){
      c3->SaveAs("plots/20160629/semileptonic/Angular_dists_starred_lep_had_fraction.pdf");
    }    
  }
  ...
  if (twod){
    ...
    TH2F *Phi_M		= (TH2F*)ftautau->Get("361510/plotEventAlgs/taus/Phi_acop_truth_obs_lep_had");

    int   NentriesPhiM	= (Phi_M->GetEntries());

    Phi_M->SetTitle("#phi_{acop}^{#tau_{lep}#tau_{had} obs.} vs. #phi_{acop}^{#tau#tau truth}");
    Phi_M->GetXaxis()->SetTitle("log_{10}#phi_{acop}^{#tau#tau truth}");
    Phi_M->GetYaxis()->SetTitle("log_{10}#phi_{acop}^{#tau_{lep}#tau_{had} obs.}");
    ...
    //save plot
    if (saveplots){
      c0->SaveAs("plots/20160629/semileptonic/Phi_acop_truth_vs_obs_lep_had.pdf");
    }
