...
void plotAngularDistRatioHadHad(){
  ...
  if (angular_fraction) {
    ...
    TH1F *h4       	= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/DPhi_truth_reco_pi_nu_had_had");
    TH1F *h5       	= (TH1F*)ftautau->Get("361510/plotEventAlgs/taus/DPhi_truth_reco_had_had");
    ...
    hist_leg->AddEntry(h4, Form("x_{4} = #Delta#phi_{#pi^{obs}#pi^{truth}}, #tau#rightarrow#nu#pi"));
    hist_leg->AddEntry(h5, Form("x_{5} = #Delta#phi_{h^{obs}h^{truth}}, other"));
    ...
    h3->GetXaxis()->SetTitle("log_{10}x_{n=1,2,3,4}");
    ...
    //save plot
    if (saveplots){
      c->SaveAs("plots/20160629/di-hadronic/Angular_dists_had_had_fraction.pdf");
    }    
  }
  ...
  if (angular_fraction2) {
    ...
    gStyle->SetTitleX(0.5);
    gStyle->SetTitleAlign(23);
    hist_leg = new TLegend(0.65,0.59,0.75,0.89);
    hist_leg->SetFillColor(0);
    hist_leg->SetLineColor(1);
    hist_leg->SetShadowColor(0);
    ...
    //save plot
    if (saveplots){
      c3->SaveAs("plots/20160629/di-hadronic/Angular_dists_starred_had_had_fraction.pdf");
    }    
  }
  ...
  if (twod){
    ...
    TH2F *Phi_M		= (TH2F*)ftautau->Get("361510/plotEventAlgs/taus/Phi_acop_truth_obs_had_had");

    int   NentriesPhiM	= (Phi_M->GetEntries());

    Phi_M->SetTitle("#phi_{acop}^{#tau_{had}#tau_{had} obs.} vs. #phi_{acop}^{#tau#tau truth}");
    Phi_M->GetXaxis()->SetTitle("log_{10}#phi_{acop}^{#tau#tau truth}");
    Phi_M->GetYaxis()->SetTitle("log_{10}#phi_{acop}^{#tau_{had}#tau_{had} obs.}");
    ...
    //save plot
    if (saveplots){
      c0->SaveAs("plots/20160629/di-hadronic/Phi_acop_truth_vs_obs_had_had.pdf");
    }
