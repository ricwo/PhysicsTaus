...
def plotEfficiency(rfiles, can, hpath):
	...
	Garray = [] # array of S_eff
	Scomp = [] # percentage of S and B
	Geff = [] # S percentage and S_eff
...
#	    print "%f %f" % (sInt, bInt)  #!!!! The first component is always (0,0) if there are N bin, there are 1+N contents with 1st compoent = 0

	    grad = sInt/bInt #signal / background 
...
#	maxBin_i = Garray.index(max(Garray))
#	gradMax = Garray[maxBin_i]
  	numBin = h.GetNbinsX()

#	print "index: %d" % minBin_i
#	print Garray[minBin_i]
#	print Scomp[minBin_i]

	        	    

        h2d = ROOT.TH2D()
        h2d.SetBins(100, 0, x, 100, 0, y)
        h2d.SetStats(False)
	...
	hS = ROOT.TH2D()
        hS.SetBins(100, 0, x , 100, 0, max(Garray))
        hS.SetStats(False)
	...
	hS.Draw('BOX')
	h2d.Draw('COL, SAME HIST')

	yaxis2 = ROOT.TGaxis(x, 0, x, y, 0, y, 100, "+L")
	yaxis2.Draw()
