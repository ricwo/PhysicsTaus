...
def getAllHistNames(rfile, hpath, hpref=''):
    ...
    for key in hdir.GetListOfKeys():
        ...
        if key.GetClassName().count('TH1') > 0 or key.GetClassName().count('TH2') > 0:
            hname = '%s%s' %(hpref, n)
...
def plot2d(rfiles, can, hpath):
	...
	ROOT.gPad.Update() #To update "z" of draw option: z means palette   

        palette = h.FindObject("palette") 
	palette.SetY1NDC(0.26) # Set Y1 value of the palette axis
	palette.SetY2NDC(0.93)
   
        if not h:
            log.warning('plot2d - failed to find h: %s:%s' %(rfile.GetName(), hpath))
            return
    ...
    if options.do_unit_area and len(hists):
        for h in hists:
            if h.Integral() > 0.0:
                ...
                log.warning('plot2d - zero integral for histogram')
...
def main():         
    ...
    regions += ['plot_prepCand_1lep_1faketau_2bjet']
    regions += ['plot_prepCand_1lep_1realtau_2bjet']
