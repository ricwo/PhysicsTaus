...
def plotEfficiency(rfiles, can, hpath):
    ...
    for rfile in rfiles:
        ...
        for i in range(h.GetNbinsX() + 1, 0, -1): # note bin number nbins + 1 is empty
            ...
            if 'BDTJetScore' in hpath:
                log.info('Bin %i: sInt = %i, bInt = %i' %(i, sInt, bInt))

        #limit = 110 * max(sInt/sDenom, bInt/bDenom)

        h2d = ROOT.TH2D()
        #h2d.SetBins(250, 0, limit, 250, 0, limit)
        h2d.SetBins(250, 0, 110 * sInt/sDenom, 250, 0, 110 * bInt/bDenom)
        #h2d.SetBins(150, 0, 110 * sInt/sDenom, 150, 0, 110 * bInt/bDenom)
        h2d.SetStats(False)
