...
def plotIntegralRatio(rfiles, can, hpath):
    ...
    for rfile in rfiles:
        ...
        if 'realtau' in hpath:
            h2path = hpath.replace('realtau', 'faketau')
        else:
            continue
        ...
        sInt = h .GetEntries() # signal integral
        bInt = h2.GetEntries() # background integral
        ...
        for i in range(middle, h.GetNbinsX() + 1): 
	#for i in range(middle, h.GetNbinsX()+1):# note bin number nbins + 1 is empty
            ...
            if i != middle:
                sInt -= h .GetBinContent(middle-(i-middle))
                bInt -= h2.GetBinContent(middle-(i-middle))

            if bInt > 0 and h2.GetBinContent(i) > 0 and h2.GetBinContent(middle-(i-middle)) > 0:
                ratio = sInt/bInt
                maxi = max(maxi, ratio)
                h.SetBinContent(i        , ratio)
                h.SetBinContent(middle-(i-middle), ratio)
            else:
                log.info('Background is zero (bin %i)' %i)
                h.SetBinContent(i, 0)
         
	
	h.SetBinContent(1,0)  #by intention
...
#    leg = getLegend(0.63, 0.70, 0.90, 0.88)
    leg = getLegend(0.33, 0.78, 0.91, 0.88) #- this is top right
    #leg = getLegend(0.105, 0.80, 0.53, 0.90)    
...
def main():         
    ...
    for r in regions:
        ...
        if 'realtau' in r:
            ...
            for h in hists:
                plotIntegralRatio(rfiles, can, '%s/%s' %(r, h))
