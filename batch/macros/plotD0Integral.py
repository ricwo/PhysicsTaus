...
def plotIntegralRatio(rfiles, can, hpath):
    ...
    for rfile in rfiles:
        ...
        for i in range(0, int(math.floor(h.GetNbinsX()/2) + 1)):
            h.SetBinContent(i, 0)
...
#    leg = getLegend(0.29, 0.28, 0.86, 0.38) #- this is top right
    ...
    ### Current cut line: ###
    line = ROOT.TLine(2.5, 0, 2.5, 2250)
    line.SetLineColor(4)
    line.SetLineStyle(2)
    line.Draw()
    ...
    ### Current cut line: ###
    if plotLine:
        line = ROOT.TLine(linex, 0, linex, 2080)
        line.SetLineColor(4)
        line.SetLineStyle(2)
        line.Draw()
