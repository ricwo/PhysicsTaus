...
def plotEfficiency(rfiles, can, hpath):
	...
	sIntMax = 0 # to count the number of total signal
	bIntMax = 0

        points = [] # normal points: sInt, bInt
	Sarray = [] # array of S_eff
	Scomp = [] # percentage of S and B
	Seff = [] # S percentage and S_eff
        
	for i in range(1, h.GetNbinsX()+1):
	    sIntMax += h .GetBinContent(i)
	    bIntMax += h2.GetBinContent(i)

#	print "%d %d" %(sIntMax, bIntMax)

        for i in range(1, h.GetNbinsX()+1): # note bin number nbins + 1 is empty
            sInt +=  h .GetBinContent(i) # factor of 2 for symmetry of D0 and Z0
            bInt +=  h2.GetBinContent(i)
	    
	    slostPercent = (sIntMax - sInt)/sIntMax
	    bgainPercent = bInt/bIntMax

	    sgainPercent = sInt/sIntMax

	    S = math.pow(slostPercent , 2) + math.pow(bgainPercent , 2)
	    print "%f" % (S)
            points += [(sInt, bInt)]
	    Sarray += [(S)]
	    Scomp += [(sgainPercent, bgainPercent)]
	    Seff += [(sgainPercent, S)]

	minBin_i = Sarray.index(min(Sarray))
  	numBin = h.GetNbinsX()

#	print "index: %d" % minBin_i
#	print Sarray[minBin_i]
#	print Scomp[minBin_i]

	        	    

        h2d = ROOT.TH2D()
        h2d.SetBins(100, 0, int(math.floor(sInt)+1), 100, 0, int(math.floor(bInt)+1))
        h2d.SetStats(False)
	h2d.GetXaxis().SetTitle("Number of Signal")
	h2d.GetXaxis().CenterTitle()
	h2d.GetXaxis().SetTitleOffset(1.5)
	h2d.GetYaxis().SetTitle("Number of Background")
	h2d.GetYaxis().CenterTitle()
	h2d.GetYaxis().SetTitleOffset(1.5)


	hpercent = ROOT.TH2D()
	hpercent.SetBins(100, 0, 1, 100, 0, 1)
	hpercent.SetStats(False)
	hpercent.GetXaxis().SetTitle("Signal efficiency")
	hpercent.GetXaxis().CenterTitle()
	hpercent.GetYaxis().SetTitle("Background efficiency")
	hpercent.GetYaxis().CenterTitle()
	hpercent.GetYaxis().SetTitleOffset(1.5)


	hS = ROOT.TH2D()
        hS.SetBins(100, 0, 1, 100, 0, 1)
        hS.SetStats(False)
	...
	for point in Scomp:
	    hpercent.Fill(point[0], point[1])
	   
	
#	hnostack = ROOT.THStack()
#	hnostack.Add(hS)
#	hnostack.Add(hpercent)
#	hnostack.Draw('COL, CONT, nostack')

#	hS.Draw('BOX')
#	hpercent.Draw('COL, SAME HIST')

#	yaxis2 = ROOT.TGaxis(1, 0, 1, 1, 0, 1, 100, "+L")
#	yaxis2.Draw()
	
#TGaxis(gpad.GetUxmax(), gpad.GetUymin(), gpad.GetUxmax(), gpad.GetUymax(), 0, rightmax, 510, "+L")

        draw_hists += [(h2d, rfile.GetName())]
#	draw_hists += [(hnostack, rfile.GetName())]
...
def main():         
    ...
    for r in regions:
        ...
        for h in hists:
            if 'NewEtTopoCone' in h and 'W1' in h:
