...
#    leg = getLegend(0.63, 0.70, 0.90, 0.88)
    #leg = getLegend(0.70, 0.64, 0.92, 0.86) # 3 samples
    #leg = getLegend(0.70, 0.68, 0.92, 0.94) # 1 sample (Z->ll)
    #leg = getLegend(0.74, 0.65, 0.99, 0.70) # ttbar
    leg = getLegend(0.73, 0.64, 0.98, 0.68) # ttbar - top right
    #leg = getLegend(0.35, 0.62, 0.60, 0.67) # ttbar - top left
#    leg = getLegend(0.68, 0.62, 0.85, 0.67)  # ttbar - bottom right
    #leg = getLegend(0.83, 0.62, 0.90, 0.67)  # ttbar - bottom right
...
def main():         
    ...
    for r in regions:
        ...
        for h in hists:
            if h.startswith('ElecTruthMatchW1') or h.startswith('MuonTruthMatchW1') or h.startswith('PromptElec') or h.startswith('PromptMuon') or h.startswith('ElecFromTau') or h.startswith('MuonFromTau') or h.startswith('TauTo'):
                plot1d(rfiles, can, '%s/plotRecoLep/%s' %(r, h))                
            else:
                plot1d(rfiles, can, '%s/%s' %(r, h))
