...
#	    print "%f" % (S)
	    ...
	    Sarray += [(S)]   #Sarray index starting from 0 to 16
	    Seff += [(sInt/sDenom, S)]
...
#	print points[minBin_i]
	     
	baxisMax = max(Sarray)   #to find the max number of y axis	    

        h2d = ROOT.TH2D()
        h2d.SetBins(100, 0, sIntMax/sDenom, 100, 0, bIntMax/bDenom)
        h2d.SetStats(False)
...
#	baxisMin = math.pow(1/(sDenom*sIntMax) , 2) + math.pow(1/(bDenom*bIntMax) , 2) 
#	print "%.25f %.25f" % (baxisMax, baxisMin)


	hS = ROOT.TH2D()
        hS.SetBins(100, 0, sIntMax/sDenom , 100, 0, bIntMax/bDenom)
        hS.SetStats(False)
	...
	yaxis2 = ROOT.TGaxis(sIntMax/sDenom, 0, sIntMax/sDenom, bIntMax/bDenom, 0, bIntMax/bDenom, 10010, "+L")
	yaxis2.Draw()
